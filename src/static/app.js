document.addEventListener("DOMContentLoaded", () => {
  ...
  async function fetchActivities() {
    try {
      ...
      // Clear loading message and reset selector options
        ...
        const participantsHtml = details.participants.length
          ? `<ul class="participants-list">${details.participants.map((participant) => `
              <li class="participant-item">
                ...
                <button class="participant-remove" data-activity="${encodeURIComponent(name)}" data-email="${encodeURIComponent(participant)}" aria-label="Remove ${participant}">
                  ✕
                </button>
              </li>
            `).join("")}</ul>`
